...
# app-level metrics
REQUESTS_TOTAL = Counter("cs3_requests_total", "Total sentiment requests received")
REQUESTS_API = Counter("cs3_requests_api", "Requests sent to HF API backend")
REQUESTS_LOCAL = Counter("cs3_requests_local", "Requests sent to local transformers backend")
LAST_LATENCY_MS = Gauge("cs3_last_latency_ms", "Latency in ms for the last request")
CSV_DOWNLOADS = Counter("cs3_csv_downloads_total", "Number of latency CSV downloads")
...
def predict_and_log(text, backend, rows_state: List[Dict[str, Any]]):
    ...
    # ==============================
    REQUESTS_TOTAL.inc()
    LAST_LATENCY_MS.set(latency_ms)
    if backend == "API (InferenceClient)":
        REQUESTS_API.inc()
    else:
        REQUESTS_LOCAL.inc()
